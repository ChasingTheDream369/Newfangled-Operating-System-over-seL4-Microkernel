...
// PROTECT~ (Place a global lock for the frame table.)
...
seL4_ARM_Page get_sos_page_cap(frame_ref_t frame_ref)
{
    frame_t *frame = frame_from_ref(frame_ref);
    return frame->sos_page;
}

seL4_ARM_Page get_user_page_cap(frame_ref_t frame_ref) {
    frame_t *frame = frame_from_ref(frame_ref);
    return frame->user_page_cap;
}
...
frame_t * get_frame_table_start() {
    return frame_table.frames;
}

size_t get_frame_table_capacity() {
    return frame_table.capacity;
}

frame_ref_t ref_from_frame(frame_t *frame)
{
    if (frame == NULL) return NULL_FRAME;
...
static frame_t *alloc_fresh_frame(void)
{
    ...
    seL4_CPtr user_page_cap = cspace_alloc_slot(frame_table.cspace);
    assert(user_page_cap != seL4_CapNull);
    seL4_Error err = cspace_copy(frame_table.cspace, user_page_cap, frame_table.cspace, sos_page, seL4_AllRights);
