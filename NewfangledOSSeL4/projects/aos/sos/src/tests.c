...
#include "../../libclock/include/clock/clock.h"
...
static void stoptimer_callback(uint32_t id, void* data) {
...
static void regular_callback_100ms(uint32_t id, void* data) {
    printf("Current time: %lu\n", get_time());
    printf("id: %u\n", id);
    printf("%s", (char*) data);
    register_timer(100000, regular_callback_100ms, data);   
}

static void regular_callback_1000ms(uint32_t id, void* data) {
    printf("Current time: %lu\n", get_time());
    printf("id: %u\n", id);
    printf("%s", (char*) data);
    register_timer(1000000, regular_callback_1000ms, data);
}

static void bad_callback(uint32_t id, void* data) {
    (void) id;
    (void) data;
    int * x = NULL;
    printf("%d\n", *x);
...
void run_post_init_tests(void) {
    ...
    printf("Current time: %lu\n", get_time());
    register_timer(1000000, regular_callback_100ms, (void*)"100ms callback data!\n"); // 1 second
    remove_timer(1000);
    register_timer(1000000, regular_callback_1000ms, (void*)"1000ms callback data!\n"); // 1 second
    register_timer(10000000, stoptimer_callback, (void*)"stop timer data!\n"); // 10 seconds
    uint32_t id = register_timer(1000000, bad_callback, (void*)"1000ms callback data!\n"); // 1 second
