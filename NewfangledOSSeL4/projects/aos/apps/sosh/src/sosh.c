...
static int showpid(int argc, char **argv) {
...
static int wait(int argc, char **argv)
{
    ...
    pid_t pid =  sos_process_wait(atoi(argv[1]));
    ...
    printf("TID of thread that exited: %d\n", pid);
...
struct command commands[] = { { "dir", dir }, { "ls", dir }, { "cat", cat }, {"cp", cp},
    { "ps", ps }, { "exec", exec }, {"sleep", second_sleep}, {"msleep", milli_sleep},
    {"time", second_time}, {"mtime", micro_time}, {"kill", kill},
    {"benchmark", benchmark}, {"showpid", showpid}, {"wait", wait},
};
